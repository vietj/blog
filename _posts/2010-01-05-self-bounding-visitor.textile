...
permalink: /2010/01/05/self-bounding-visitor/index.html
post_id: 158
categories: 
- Java
- Uncategorized
...
<pre name="code" class="java">abstract class Node<E extends Node<E>> {
   public E left;
   public E right;
}
...
<pre name="code" class="java">FooNode root = new FooNode();
root.left = new FooNode();
root.right = new BarNode();</pre>
But that is not possible:
<pre name="code" class="java">FooNode root = new FooNode();
root.left = new JuuNode();</pre>
...
<pre name="code" class="java">interface Visitor<V extends Visitor<V,S>, S extends Strategy<V,S>> {
  // The interface is empty as the base visitor does not provide an API
}</pre>
<pre class="java">interface Strategy<V extends Visitor<V,S>, S extends Strategy<V,S>> {
  void visit(Type type, V visitor);
}</pre>
Now we can define the hierarchy visitor and its strategy
<pre name="code" class="java">interface HierarchyVisitor<T extends HierarchyVisitor<T>> extends
...
<pre name="code" class="java">enum HierarchyScope {
  ALL() {
